...
This API is used to mid inquiry. Merchants needs to add session token received during login API call in the header of this API.
...
The below table contains the mandatory fields required for a successful request. The full request schemas are available in our [API Explorer](../api/?type=get&path=/Externalboarding/secure/refmid/{appType}/{refmid}`).
